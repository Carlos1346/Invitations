...
import { Container, Row, Col, Card, ListGroup, Button, Spinner } from "react-bootstrap"; // Importar Button desde react-bootstrap
  ...
  useEffect(function fetchEvents() {
    async function fetchData() {
      try {
        ...
        const eventsWithAttendance = response.data.map(function (event) {
          return {
            ...event,
          ...
          return (
            <Col key={event.id} xs={12} md={6} lg={4}>
              <Card className="mb-4">
                ...
                />
                <Card.Body>
                  <Card.Title>Nombre del evento: {event.event_name}</Card.Title>
                  <Card.Text>Descripcion: {event.event_description}</Card.Text>
                </Card.Body>
                <ListGroup className="list-group-flush">
                  <ListGroup.Item>Direccion: {event.event_address}</ListGroup.Item>
                  <ListGroup.Item>
